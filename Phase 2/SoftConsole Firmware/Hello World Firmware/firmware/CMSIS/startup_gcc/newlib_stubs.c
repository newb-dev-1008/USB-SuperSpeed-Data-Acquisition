...
/* #define SEMIHOSTING
 *
 * Uncomment above line to use semihosting functionality. Add the
 * --specs=rdimon.specs in linker options.
 */
#ifndef SEMIHOSTING
/*==============================================================================
 * Redirection of standard output to a CoreUARTapb instance.
 *------------------------------------------------------------------------------
 * A default implementation for the redirection of the output of printf() and
 * scanf() is provided in this file. This redirection is enabled by
 ...
 * definition of MSCC_STDIO_BAUD_VALUE to the project with the value set to the
 * required baud rate
 * Instructions to use:
 ...
 * 3. The default baud rate of 115200 can be modified by defining
 *    MSCC_STDIO_BAUD_VALUE with the desired baud rate in your project.
 */
...
int sendchar(int ch)
{
    ...
    */
    if ( !g_stdio_uart_init_done )
    {
         ...
         *********************************************************************/
        UART_init( &g_stdio_uart, MSCC_STDIO_UART_BASE_ADDR,
                   SYS_M1_CLK_FREQ/((16 * MSCC_STDIO_BAUD_VALUE)-1),
                   (DATA_8_BITS | NO_PARITY));
        g_stdio_uart_init_done = 1u;
    }
...
int _read_r( struct _reent *err, int file, char *ptr, int len)
{
    if (file != STDIN_FILENO)
    {
        errno = EBADF;
 ...
 */
int _write_r( struct _reent *err, int file, char * ptr, int len )
{
#ifdef MSCC_STDIO_THRU_CORE_UART_APB
    int count_out;
    /*--------------------------------------------------------------------------
...
caddr_t _sbrk(int incr)
{
    ...
    if(heap_end < stack_ptr)
    {
        ...
        {
          const char* str = "Heap and stack collision\n";
          _write (1, str, 25);
          _exit (1);
        }
    }
